...
#ifdef MEDIA_WORKSHOP_ENABLED
#include <media-workshop/MediaWorkshopDock.hpp>
#endif
...
#ifdef MEDIA_WORKSHOP_ENABLED
	mediaWorkshopDock = new MediaWorkshopDock();
	AddDockWidget(mediaWorkshopDock, Qt::RightDockWidgetArea);
	mediaWorkshopDock->setVisible(false);
#endif
...
#ifdef MEDIA_WORKSHOP_ENABLED
	if (mediaWorkshopDock && !mediaWorkshopDock->ConfirmShutdown()) {
		event->ignore();
		restart = false;
		return;
	}
